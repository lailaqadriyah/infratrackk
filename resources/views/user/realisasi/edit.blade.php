...
<div class="p-6 bg-gray-50 min-h-screen w-full">
    <div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route('user.realisasi.update', $realisasi->id) }}" method="POST" class="p-6 space-y-6">
            ...
            <!-- Kegiatan -->
            <div>
                <label for="kegiatan" class="block text-sm font-semibold text-gray-700 mb-2">Kegiatan</label>
                <input type="text" name="kegiatan" id="kegiatan" value="{{ old('kegiatan', $realisasi->kegiatan) }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('kegiatan') border-red-500 @enderror">
                @error('kegiatan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
