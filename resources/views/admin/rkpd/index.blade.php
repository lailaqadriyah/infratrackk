...
<div x-data="rkpdManager()" x-init="init()" class="space-y-0">
...
<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="p-4 border-b border-gray-200">
        <h3 class="font-semibold text-gray-900">Data RKPD</h3>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($rkpds as $rkpd)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">Rp {{ number_format($rkpd->anggaran ?? 0, 0, ',', '.') }}</td>
                        @php
                            $editDataRkpd = json_encode([
                                'id' => $rkpd->id,
                                'id_tahun' => $rkpd->id_tahun,
                                'id_opd' => $rkpd->id_opd,
                                'program' => $rkpd->program ?? '',
                                'kegiatan' => $rkpd->kegiatan ?? '',
                                'anggaran' => $rkpd->anggaran ?? 0,
                            ]);
                        @endphp
                        <td class="px-6 py-4 text-sm">
                            <div class="flex items-center gap-2">
                                <button type="button" class="btn-edit-rkpd p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit" data-rkpd="{{ e($editDataRkpd) }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <form id="delete-form-rkpd-{{ $rkpd->id }}" action="{{ route('admin.rkpd.destroy', $rkpd) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                <button type="button" class="btn-delete-rkpd p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus" data-id="{{ $rkpd->id }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div x-show="uploadModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @keydown.escape.window="uploadModalOpen = false">
    <div @click.outside="uploadModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-lg w-full overflow-hidden">
        ...
        <form action="{{ route('admin.rkpd.upload') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            <p class="text-sm text-gray-600">Upload file Excel (.xls, .xlsx) untuk import data. Pastikan kolom sesuai: program, kegiatan, anggaran</p>
            <div>
...
<div x-show="editModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 overflow-y-auto" @keydown.escape.window="editModalOpen = false">
    <div @click.outside="editModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-2xl w-full my-8 overflow-hidden">
        ...
        <form id="edit-form-rkpd" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
...
function attachRkpdHandlers() {
    // Edit buttons
    document.querySelectorAll('.btn-edit-rkpd').forEach(function(btn) {
        btn.addEventListener('click', function() {
            var data = JSON.parse(this.dataset.rkpd);
            if (rkpdAlpine) {
                rkpdAlpine.editModalOpen = true;
            }
            setTimeout(function() {
                var form = document.getElementById('edit-form-rkpd');
                if (form) {
                    form.action = '{{ url("/admin/rkpd") }}/' + data.id;
    ...
    // Delete buttons
    document.querySelectorAll('.btn-delete-rkpd').forEach(function(btn) {
        btn.addEventListener('click', function() {
