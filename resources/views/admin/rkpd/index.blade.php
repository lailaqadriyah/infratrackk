...
<div x-data="rkpdManager()" class="space-y-0">
...
<div class="flex gap-6 mb-6 border-b border-gray-200">
    ...
    <a href="{{ route('admin.rkpd.index') }}" class="pb-4 font-medium text-blue-600 border-b-2 border-blue-600">RKPD</a>
</div>
...
<div class="bg-white rounded-lg shadow-sm p-4 mb-6">
    <form method="GET" action="{{ route('admin.rkpd.index') }}" class="flex flex-wrap items-end gap-4">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.rkpd.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-sm">Reset</a>
        </div>
    </form>
</div>
...
<div class="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <h3 class="font-bold text-gray-900 flex items-center gap-2">
            ...
            Data Rencana Kerja Pemerintah Daerah (RKPD)
        </h3>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full border-collapse">
            <thead class="bg-blue-100 border-b-2 border-gray-300">
                <tr>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">Tahun</th>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">OPD</th>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">Program</th>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">Kegiatan</th>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">Anggaran</th>
                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider border-x border-gray-300">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-300">
                @forelse ($rkpds as $rkpd)
                    <tr class="hover:bg-green-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-black-900 border-x border-gray-200">{{ $rkpd->tahun->tahun ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-black-900 border-x border-gray-200">{{ $rkpd->opd->nama_opd ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-black-600 border-x border-gray-200">{{ $rkpd->program ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-black-600 border-x border-gray-200">{{ $rkpd->kegiatan ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-black-900 border-x border-gray-200">
                            Rp {{ number_format($rkpd->anggaran ?? 0, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 text-sm border-x border-gray-200">
                            <div class="flex justify-center items-center gap-2">
                                {{-- PERBAIKAN: Tombol edit langsung memicu fungsi openEditModal --}}
                                <button type="button" 
                                    @click="openEditModal({
                                        id: {{ $rkpd->id }},
                                        id_tahun: {{ $rkpd->id_tahun }},
                                        id_opd: {{ $rkpd->id_opd }},
                                        program: '{{ addslashes($rkpd->program) }}',
                                        kegiatan: '{{ addslashes($rkpd->kegiatan) }}',
                                        anggaran: {{ $rkpd->anggaran ?? 0 }}
                                    })"
                                ...
                                <button type="button" @click="confirmDelete({{ $rkpd->id }})" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                                <form id="delete-form-rkpd-{{ $rkpd->id }}" action="{{ route('admin.rkpd.destroy', $rkpd) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-gray-500">Belum ada data. Tambahkan data dengan tombol di atas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="mt-6">{{ $rkpds->links() }}</div>
...
<div x-show="uploadModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @keydown.escape.window="uploadModalOpen = false">
    <div @click.outside="uploadModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-lg w-full overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-bold text-gray-900">Upload File RKPD</h3>
        ...
        <form action="{{ route('admin.rkpd.upload') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            <div>
...
<div x-show="manualModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 overflow-y-auto" @keydown.escape.window="manualModalOpen = false">
    <div @click.outside="manualModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-2xl w-full my-8 overflow-hidden">
        ...
        <form action="{{ route('admin.rkpd.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
...
<div x-show="editModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 overflow-y-auto" @keydown.escape.window="editModalOpen = false">
    <div @click.outside="editModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-2xl w-full my-8 overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-bold text-gray-900">Edit Data RKPD</h3>
            <button type="button" @click="editModalOpen = false" class="p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>
        <form :action="'{{ url('admin/rkpd') }}/' + editData.id" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
        ...
        confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-form-rkpd-' + id).submit();
                }
            });
        }
    };
