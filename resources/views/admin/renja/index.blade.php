...
<div x-data="renjaManager()" class="space-y-0">
                            ...
                            <div class="flex justify-center items-center gap-1">
                                ...
                                <button type="button" 
                                    @click="openEditModal({
                                        ...
                                        id_opd: {{ $renja->id_opd }},
                                        program: '{{ addslashes($renja->program) }}',
                                        kegiatan: '{{ addslashes($renja->kegiatan) }}',
                                        sub_kegiatan: '{{ addslashes($renja->sub_kegiatan) }}',
                                        indikator: '{{ addslashes($renja->indikator) }}',
                                        target: '{{ addslashes($renja->target) }}',
                                ...
                                <button type="button" @click="confirmDelete({{ $renja->id }})" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                                <form id="delete-form-{{ $renja->id }}" action="{{ route('admin.renja.destroy', $renja) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
...
<div x-show="editModalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 overflow-y-auto" @keydown.escape.window="editModalOpen = false">
    <div @click.outside="editModalOpen = false" class="bg-white rounded-xl shadow-xl max-w-2xl w-full my-8 overflow-hidden">
        ...
        <form :action="'{{ url('admin/renja') }}/' + editData.id" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
