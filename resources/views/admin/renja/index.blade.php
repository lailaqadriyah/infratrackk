...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900">Modul Perencanaan</h1>
...
<div class="flex gap-3 mb-6">
    ...
    <a href="{{ route('admin.renja.create') }}" class="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($renjas as $renja)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">Rp {{ number_format($renja->anggaran, 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-sm">
                            <div class="flex gap-2">
                                <a href="{{ route('admin.renja.edit', $renja->id) }}" class="text-blue-600 hover:text-blue-700 font-medium text-xs">Edit</a>
                                <form method="POST" action="{{ route('admin.renja.destroy', $renja->id) }}" class="inline" onsubmit="return confirm('Yakin ingin menghapus?');">
                                    @csrf
                                    @method('DELETE')
