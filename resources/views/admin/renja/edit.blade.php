...
@section('title', 'Edit RENJA')

@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900">Edit Data RENJA</h1>
    <p class="text-gray-600 mt-2">Ubah data RENJA yang ada</p>
</div>

<div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
    <form action="{{ route('admin.renja.update', $renja->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="id_tahun" id="id_tahun" class="w-full border border-gray-300 rounded-lg px-4 py-2 @error('id_tahun') border-red-500 @enderror" required>
                    ...
                    @foreach ($tahuns as $tahun)
                        <option value="{{ $tahun->id }}" {{ $renja->id_tahun == $tahun->id ? 'selected' : '' }}>{{ $tahun->tahun }}</option>
                    @endforeach
                </select>
                @error('id_tahun')
